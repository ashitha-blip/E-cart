...
function Cart() {
  ...
  useEffect(()=>{
    if(cart?.length>0){
      setTotal(cart?.map(product=>product?.totalPrice).
    reduce((p1,p2)=>p1+p2))
    }else{
      setTotal(0)
    }
    ...
    <div>
        ...
        <div className="table shadow mt-5">
          <table className='w-100'>
            ...
            <tr>
              ...
              <td>
                <input type="text" readOnly value={product.quantity} style={{width:"25px"}}/>
              </td>
      ...
      <div className="col-lg-3">
        <div className=" card shadow rounded mt-5 p-5 w-100">
          ...
          <h3>
                  <span className="text-dark fw-bolder">Total Products</span>{cart?.length}
                </h3>
                <h3>
                  Total Price:{" "}
